...
parser.add_argument("--train_mode", type=str, default='regression', required=False)  # regression    
...
def evaluate(model, eval_loader, optimizer, loss_fn, epoch):
    global max_acc,Has0_acc_2,Has0_F1_score,Non0_acc_2,Acc_3,Mult_acc_2,Mult_acc_3,Mult_acc_5,F1_score,MAE,Corr,F1_score_5
    ...
    for cur_iter, sample in eval_pbar:
        ...
        if args.train_mode == 'classification':
            label = sample['labels']['M'].long().squeeze()
        else:
            label = sample['labels']['M'].squeeze()
    ...
    save_excel(all_feature_list, './output/tsne/{}//all_feat_1.xlsx'.format(args.datasetName))
    save_excel(all_label, './output/tsne/{}/all_label_1.xlsx'.format(args.datasetName))
