...
project_name = "CodeTest0517_mixup" 
...
def main():
    ...
    model = MyMultimodal_Integrally(num_classs = 3,
                        visual_seq_len=50,
                        audio_seq_len=50,
                        batchsize=64,
                        trans_depth=2,
    ...
    input = torch.randn(64,20,20).to(device)
    input2= torch.randn(64,20,5).to(device)
...
def train(model, train_loader, optimizer, loss_fn, epoch, writer):
    ...
    for cur_iter, sample in train_pbar:
        ...
        # # exit(0)
        from thop import profile
        input = torch.randn(64,20,768).to(device)
        input2= torch.randn(64,20,5).to(device)
        input3=torch.randn(64,20,20).to(device)
        flops, params = profile(model, inputs=(input,input2,input3))
        print('FLOPs = ' + str(flops/1000**3) + 'G')
        print('Params = ' + str(params/1000**2) + 'M')
