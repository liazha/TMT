...
from models.MyModel_Ablation import MMT_semi
import argparse
...
def train(model, train_loader, optimizer, loss_fn, epoch, writer):
    ...
    for cur_iter, sample in train_pbar:
        ...
        # label = torch.where(label>=2, torch.tensor(6.), label)
        # label = torch.where(label>=1 and label<2, torch.tensor(5.), label)
        # label = torch.where(label>=0 and label<1, torch.tensor(4.), label)
        # label = torch.where(label>=-1 and label<0, torch.tensor(3.), label)
        # label = torch.where(label>=-2 and label<-1, torch.tensor(2.), label)
        # label = torch.where(label>=-3 and label<-2, torch.tensor(1.), label)
        # label = label.squeeze().long().to(device)
        # print(label)
        # exit(0)
        x_vision, x_audio, x_text, lebel_a, lebel_b, lam = mixup_data(x_vision, x_audio, x_text, label, 1.0,
                                                                      device=device)
        ...
        # stat(model,input_a)
        # exit(0)
        D0_visual_op, D0_audio_op, D0_text_op, D2_visual_op, D2_audio_op, D2_text_op, cls_output, a, b, c = model(
...
def evaluate(model, eval_loader, optimizer, loss_fn, epoch, writer, save_path):
    ...
    for cur_iter, sample in eval_pbar:
        ...
        with torch.no_grad():
            D0_visual_op, D0_audio_op, D0_text_op, D2_visual_op, D2_audio_op, D2_text_op, cls_output, a, b, c = model(
                x_vision.to(device), x_audio.to(device), x_text.to(device))
    ...
    if args.train_mode == 'classification':
        ...
        # if eval_results['Acc_3'] > max_acc:
        #             max_acc = eval_results['Has0_acc_2']
        #             Has0_acc_2 = eval_results['Has0_acc_2']
        #             Non0_acc_2 = eval_results['Non0_acc_2']
        #             Acc_3 = eval_results['Acc_3'] 
        #             Has0_F1_score = eval_results['Has0_F1_score']
        ...
        if eval_results['Has0_acc_2'] > max_acc:
            max_acc = eval_results['Has0_acc_2']
            Has0_acc_2 = eval_results['Has0_acc_2']
            Non0_acc_2 = eval_results['Non0_acc_2']
            Acc_3 = eval_results['Acc_3']
        ...
        print("eval_results; Has0_acc_2: ", Has0_acc_2, "Non0_acc_2: ", Non0_acc_2, "Has0_F1_score:", Has0_F1_score,
              "Acc_3: ", Acc_3, "F1_score_3: ", F1_score_3)
        if eval_results['Acc_3'] > 0.6937:
            # if eval_results['Acc_3'] > 0.80:
            save_model(save_path, epoch, model, optimizer, eval_results['Has0_acc_2'], eval_results['Acc_3'],
                       eval_results['Has0_F1_score'], 2)

        # if eval_results['Has0_acc_2'] > 0.80 or eval_results['Acc_3'] > 0.68 or eval_results['Has0_F1_score'] > 0.8191:
        #     save_model(save_path, epoch, model, optimizer, eval_results['Has0_acc_2'], eval_results['Acc_3'], eval_results['Has0_F1_score'], 2)
        # if eval_results['Acc_3'] > 0.6915:
        #     save_model(save_path, epoch, model, optimizer, eval_results['Has0_acc_2'], eval_results['Acc_3'], eval_results['Has0_F1_score'], 2)
        # if eval_results['Has0_F1_score'] > 0.80:
        #     save_model(save_path, epoch, model, optimizer, eval_results['Has0_acc_2'], eval_results['Acc_3'], eval_results['Has0_F1_score'], 2)
    else:
        ...
        print("eval_results; Mult_acc_2: ", Mult_acc_2, "Mult_acc_3: ", Mult_acc_3, "Mult_acc_5:", Mult_acc_5,
              "F1_score: ", F1_score, "MAE: ", MAE, "Corr: ", Corr)

        # if eval_results['Mult_acc_2'] > 0.80 or eval_results['Mult_acc_3'] > 0.6915 or eval_results['Mult_acc_5'] > 0.40 or eval_results['F1_score'] > 0.8191 or eval_results['MAE'] < 0.42 or eval_results['Corr'] > 0.56:
        #     save_model_reg(save_path, epoch, model, optimizer, eval_results['Mult_acc_2'], eval_results['Mult_acc_3'], eval_results['Mult_acc_5'], eval_results['F1_score'], eval_results['MAE'], eval_results['Corr'], 2)
